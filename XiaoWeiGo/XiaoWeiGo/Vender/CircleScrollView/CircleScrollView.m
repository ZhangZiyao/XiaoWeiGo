...
//  Erba
//
//  Created by 一心 on 2017/4/13.
//  Copyright © 2017年 一心·张梓瑶. All rights reserved.
//
...
//创建用来缓存图片的文件夹
+ (void)initialize {
    cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"CircleScrollView"];
    BOOL isDir = NO;
...
//            [self downloadImages:i];
            [_images addObject:[UIImage imageNamed:imageArray[i]]];
        }
    }
...
- (void)setDescribeTextColor:(UIColor *)color font:(UIFont *)font backgroundColor:(UIColor *)backgroundColor {
    if (color) self.describeLabel.textColor = color;
    if (font) self.describeLabel.font = font;
    if (backgroundColor) self.describeLabel.backgroundColor = backgroundColor;
}
...
- (void)imageClick {
    ...
    } else if ([_delegate respondsToSelector:@selector(circleScrollView:clickImageAtIndex:)]){
        [_delegate circleScrollView:self clickImageAtIndex:self.currIndex];
    }
}
