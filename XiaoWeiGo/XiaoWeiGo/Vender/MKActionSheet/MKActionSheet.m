...
- (void)initData{
    ...
    _titleColor = UIColorFromRGB16(0x666666);
    _titleFont = [UIFont rw_regularFontSize:15];
    _titleMargin = 20.0f;
    
    _buttonTitleColor = UIColorFromRGB16(0x666666);
    _buttonTitleFont = [UIFont rw_regularFontSize:18.0f];
    _buttonOpacity = 1;
    _buttonImageRightSpace = 12.f;
    
    _destructiveButtonIndex = -1;
    _destructiveButtonTitleColor = mainColor;
    
    _selectedButtonTitleColor = mainColor;
    
    _needCancelButton = YES;
    _cancelTitle = @"取消";
    
    //以 object array 初始化，默认没有箭头图片
    if (_paramIsObject) {
        self.showAccessory = NO;
    }
    
    _accessoryImage = @"right";
//    _accessoryImageView.image = [UIImage imageNamed:@"right"];
...
//            [self.titleView addSubview:self.accessoryImageView];
//        }
        if (self.selectType == MKActionSheetSelectType_multiselect) {
...
//                        make.right.equalTo(self.sheetView).offset(-35*kScaleW);
//                        make.centerY.equalTo(self.sheetView);
//        }];
    }
//    if (self.showAccessory) {
...
//    [selectBtn setImage:selectImg2 forState:UIControlStateDisabled];
    [selectBtn setImage:[UIImage imageWithColor:mainColor size:CGSizeMake(3, self.currentConfig.buttonHeight/2)] forState:UIControlStateSelected];
    [selectBtn setImage:[UIImage imageWithColor:mainColor size:CGSizeMake(3, self.currentConfig.buttonHeight/2)] forState:UIControlStateNormal];
    return selectBtn;
}
