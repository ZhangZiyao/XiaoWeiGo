...
- (void)layoutSubviews {
    ...
    _attributesNormal = @{NSFontAttributeName: [UIFont systemFontOfSize:15],
                          NSForegroundColorAttributeName: _normalTextColor};
    _attributesSelected = @{NSFontAttributeName: [UIFont systemFontOfSize:15],
...
//    if (self.delegate) {
        [self.delegate didSelectedAtIndex:newIndex didDeselectAtIndex:oldIndex];
//    }
    ...
    if (_contentType == YUSegmentedControlContentTypeText) {
        ...
        for (YUSegmentedControlItem *item in _items) {
            UILabel *ii = (UILabel *)item.view;
            if (ii != selectedLabel) {
                ii.attributedText = [[NSAttributedString alloc] initWithString:ii.text attributes:_attributesNormal];
            }
        }
    ...
    if (_showsIndicator != showsIndicator) {
        ...
        if (showsIndicator) {
            _indicator = ({
                ...
                indicator.backgroundColor = navColor;
